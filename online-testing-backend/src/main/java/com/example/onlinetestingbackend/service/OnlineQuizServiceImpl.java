...
//    }
    ...
    public List<OqCourseForStudentDTO> getCoursesByStudent(Integer studentId) {
        ...
        Map<Integer, Integer> courseTeacherMap = sections.stream()
                .collect(Collectors.toMap(
                        Section::getCourseId,
                        Section::getTeacherId,
        ...
        // 构建 courseId -> Section 映射以便后续获取 sectionId
        Map<Integer, Section> courseIdToSectionMap = sections.stream()
                .collect(Collectors.toMap(Section::getCourseId, s -> s));

        // 4. 查询课程详细信息并转换为DTO
        return courseRepository.findAllById(courseTeacherMap.keySet()).stream()
                .map(course -> {
                    // 获取对应的 section
                    Section section = courseIdToSectionMap.get(course.getCourseId());
                    if (section == null) {
                        throw new IllegalArgumentException("找不到对应的教学班");
                    }
